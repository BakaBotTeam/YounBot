...
public class HypixelCommand
{
    ...
    public async void Hypixel(BotContext context, MessageChain chain, string name)
    {
        try
        {
            ...
            var basicBuilder = MessageBuilder.Group(chain.GroupUin!.Value).Text(
                "Hypixel 玩家数据:\n" +
                $"玩家名: {playerName}\n" +
                $"等级: {level}\n" +
                ...
                $"当前状态: {stringOnlineStatus}").Time(DateTime.Now);
            var bedwarsBuilder = MessageBuilder.Group(chain.GroupUin!.Value).Text("Bedwars信息获取失败!").Time(DateTime.Now)
                .Build();
            var skywarsBuilder = MessageBuilder.Group(chain.GroupUin!.Value).Text("Skywars信息获取失败!").Time(DateTime.Now)
                .Build();
            var duelBuilder = MessageBuilder.Group(chain.GroupUin!.Value).Text("Duel信息获取失败!").Time(DateTime.Now)
                .Build();
            if (playerInfo.ContainsKey("stats"))
            {
                ...
                if (playerStats.ContainsKey("Bedwars"))
                {
                    ...
                    bedwarsBuilder = MessageBuilder.Group(chain.GroupUin!.Value).Text("Bedwars 信息:\n" +
                        ...
                        $"最终击杀数: {bwStats.GetIntOrNull("final_kills_bedwars")}").Time(DateTime.Now).Build();
                }

                if (playerStats.ContainsKey("SkyWars"))
                {
                    var swStats = playerStats.GetObject("SkyWars");
                    skywarsBuilder = MessageBuilder.Group(chain.GroupUin!.Value).Text("Skywars 信息:\n" +
                ...
                if (playerStats.ContainsKey("Duels"))
                {
                    ...
                    duelBuilder = MessageBuilder.Group(chain.GroupUin!.Value).Text("Duel 信息:\n" +
        ...
        {
            await SendMessage(context, chain, exception.Message, true);
        }
    }
